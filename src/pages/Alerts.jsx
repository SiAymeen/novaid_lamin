...
function formatDate(dateStr, t) {
  if (!dateStr) return t('alerts.neverVisited');
  ...
  if (days === 1) return t('alerts.yesterday');
  if (days < 7) return t('alerts.daysAgo', { count: days });
          ...
          <section className="lg:col-span-2">
            ...
            <div className="space-y-3 mb-8">
              ...
              {urgentFamilies.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-primary">{t('alerts.urgentSituations')}</h3>
                    ...
                    <div key={f._id} className="card card-left-border urgent">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Link to={`/families/${f._id}`} className="btn btn-outline">
                            {t('common.viewFile')}
                          </Link>
                          <button onClick={() => handlePlanVisit(f)} className="btn btn-primary">
                            {t('alerts.planVisit')}
                          </button>
                        </div>
                      </div>
              ...
              {forgottenFamilies.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-primary">{t('alerts.forgottenVisits')}</h3>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-muted flex items-center gap-1">
                            <Clock size={14} />
                            {t('alerts.lastVisit')}: {formatDate(f.lastVisitDate, t)}
                          </p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          <Link to={`/families/${f._id}`} className="btn btn-outline">
              ...
              {urgentFamilies.length + forgottenFamilies.length === 0 && (
                <div className="card">
          ...
          <div className="flex flex-col gap-6">
            ...
            <section>
              <h2 className="text-lg font-semibold text-primary mb-4 flex items-center gap-2">
                ...
                {t('alerts.criticalStock')}
              </h2>
              
              <div className="card">
                {lowStockItems.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-green-500 font-medium">✅ {t('alerts.stockOK')}</p>
                  </div>
                ) : (
            ...
            <section>
              <h2 className="text-lg font-semibold text-primary mb-4 flex items-center gap-2">
                ...
                {t('alerts.recentReports')}
              </h2>
              
              <div className="card">
                {recentReports.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-secondary">{t('alerts.noReports')}</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {recentReports.map((r) => (
                      <div key={r._id} className="p-3 rounded-lg border border-blue-500/20 bg-blue-500/5 hover:bg-blue-500/10 transition-colors">
                        <div className="flex items-start justify-between mb-1">
                          ...
                          <span className="text-xs text-muted">{formatDate(r.date, t)}</span>
                        </div>
